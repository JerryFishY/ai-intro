...
def b1(a):
    ...
    输出： 一个整数
    '''
    ans = sum([a[i] for i in range(0, len(a)) if i%2 == 0])
    return ans
...
def b2(scores:dict):
    ...
    '''
    ret = [name for name, score in scores.items() if score > 50]
    ret.sort()
    return ret        
...
def q2(n):
    ...
    '''
    x = np.fromfunction(lambda i, j: i+j, (n, n))
    return x
...
def q3(name_score_dict):
    ...
    res = [Student(name, score) for name, score in name_score_dict.items()]
...
def q5():
    ...
    '''
    sample_num = 10000
    x,y = np.random.uniform(0, 1, (sample_num)),np.random.uniform(0, 1, (sample_num))
    res = 4 * np.sum(x**2 + y**2 < 1) / sample_num
    return res
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        # 你的实现代码
        LayoutText = [[' ' for _ in range(self.width)] for _ in range(self.height)]
        LayoutText[self.pacman_pos[0]][self.pacman_pos[1]] = 'P'
        for x, y in self.walls:
            LayoutText[x][y] = '%'
        for x, y in self.foods:
            LayoutText[x][y] = '.'
        LayoutText = '\n'.join([''.join(i) for i in LayoutText])
        return LayoutText
...
def q6(file_path):
    ...
    '''
    res = Layout.build_layout(file_path)
    return res
